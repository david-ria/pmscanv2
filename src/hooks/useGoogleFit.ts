...
export function useGoogleFit(): UseGoogleFitReturn {
      ...
      window.gapi.load('auth2', {
        callback: async () => {
          try {
            ...
            const { data: clientIdResponse, error: clientIdError } =
              await supabase.functions.invoke('get-google-client-id');

            if (clientIdError) {
              console.error(
                'Error calling get-google-client-id function:',
                clientIdError
              );
              throw new Error(
                `Failed to get Google Client ID: ${clientIdError.message}`
              );
            }
            ...
            logger.debug(
              'Received client ID:',
              clientId ? 'Got client ID' : 'No client ID'
            );

            if (!clientId) {
              throw new Error(
                'Google Client ID not configured in Supabase secrets'
              );
            }
  ...
  const getRecentActivity = useCallback((): GoogleFitActivity | null => {
    ...
    const recentActivity = activities.find(
      (activity) => new Date(activity.end_time).getTime() > oneHourAgo
    );
