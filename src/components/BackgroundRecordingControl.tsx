...
import { Moon, Sun, Smartphone, Wifi, Bell, BatteryLow, Shield } from "lucide-react";
...
export function BackgroundRecordingControl() {
  const [isBackgroundMode, setIsBackgroundMode] = useState(getBackgroundRecording());
    ...
    if (enabled) {
      if (!backgroundSyncSupported) {
        toast({
          ...
          description: "Your browser doesn't support background sync. Data will only be recorded when the app is active.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (notificationPermission !== 'granted') {
        ...
        if (!granted) {
          toast({
            ...
            description: "Enable notifications to get alerts if background recording stops.",
            variant: "destructive",
          });
        }
      }
      ...
      toast({
        ...
        description: "PMScan will continue recording even when the app is in the background.",
      });
    } else {
        ...
        {isBackgroundMode && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="flex items-center gap-2 p-2 border rounded">
              <Wifi className={`h-4 w-4 ${backgroundSyncSupported ? 'text-green-500' : 'text-red-500'}`} />
            ...
            <div className="flex items-center gap-2 p-2 border rounded">
              <Bell className={`h-4 w-4 ${notificationPermission === 'granted' ? 'text-green-500' : 'text-amber-500'}`} />
              <span className={getStatusColor(notificationPermission === 'granted')}>
                Notifications {getStatusIcon(notificationPermission === 'granted')}
              </span>
            </div>
            
            <div className="flex items-center gap-2 p-2 border rounded">
              <BatteryLow className={`h-4 w-4 ${wakeLock ? 'text-green-500' : 'text-amber-500'}`} />
            ...
            <div className="flex items-center gap-2 p-2 border rounded">
              <Shield className={`h-4 w-4 ${isBackgroundEnabled ? 'text-green-500' : 'text-gray-500'}`} />
        ...
        {!backgroundSyncSupported && (
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-sm text-amber-800">
              <strong>Limited Support:</strong> Your browser doesn't support background sync. 
              Recording will pause when the app is closed or minimized.
            </p>
          </div>
        )}
          ...
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-blue-800">
                <strong>Background Recording Active:</strong> Your PMScan data is being collected 
